    ...
    if engine != 'deepcut':
        ...
        if mode == 'dg':
            try:
                ...
                print(engine_type)
                model_load.load_weights(f'tws_cut/weight/model_weight_{engine_type}.h5')
            except:
                print()
                raise Exception('Error Engine TL-XXXX-CORPUS_NAME')  
        else: 
            model_load.load_weights(f'tws_cut/model/ds_weights/{engine}_{opt_name}_{mode}_{lstm_node}_{attension_node}_weight.h5')
